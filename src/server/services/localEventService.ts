...
function readEventMetadata(metadataPath: string) {
...
export function getLocalEvents(): Event[] {
  ...
  return fs
    .readdirSync(EVENTS_PATH, { withFileTypes: true })
...
export function getLocalEventBySlug(slug: string) {
  return getLocalEvents().find((event) => event.slug === slug) ?? null;
}
